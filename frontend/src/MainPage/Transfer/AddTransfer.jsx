...
const AddTransfer = () => {
  ...
  const transferDatas = { from, to, startDate, orderTax, totalDiscount, shipping, description, grandTotalNumber, status, count, data }
  ...
  const addTransfer = async (e) => {
    try {
      ...
      const response = await axios.post(`${peopleUrl}/add-transfer`, transferDatas);
    ...
    try {
      await axios.post(`${productUrl}/filtered-data?sku=${sku}`).then((response) => {
        const newData = response.data.map(product => ({
          ...product,
          count: 1, // Initial count for each product
        }));
        setGrandTotalNumber(prevTotal => prevTotal + newData.reduce((acc, cur) => acc + cur.subTotal, 0));
        if (data.length === 0) {
  ...
  const incrementCount = (index) => {
    ...
    updatedCounts[index] = (updatedCounts[index] || 1) + 1;
  ...
  const deleteRow = (_id) => {
    ...
    const subTotal = data[index].subTotal;
    ...
    <>
      <div className="page-wrapper">
        <div className="content">
          ...
          <form action="" onSubmit={addTransfer}>
            <div className="card">
